...
extern QString scoreInfo; //itufo
void collectTimeline(const Note* note, int pitch, int velo, int onTick, int offTick)
    {
        //itufo
        //QPointF pos(note->pagePos()+note->pos());
        QPointF pos(note->pagePos());

        scoreInfo.append(QString("\n{\"id\":%1").arg(note->getId()));
        scoreInfo.append(QString(", \"times\":%1")
                         .arg( const_cast<Note*>(note)->incrTimes()));
        scoreInfo.append(QString(", \"line\":%1, \"pitch\":%2")
                         .arg(note->line())
                         .arg(pitch));
        scoreInfo.append(QString(", \"velo\":%1")
                         .arg(velo));        
        scoreInfo.append(QString(", \"ontime\":%1, \"offtime\":%2")
                         .arg(note->score()->tempomap()->tick2time(onTick, 0))
                         .arg(note->score()->tempomap()->tick2time(offTick+1, 0)));
        scoreInfo.append(QString(", \"ontick\":%1, \"offtick\":%2")
                         .arg(onTick)
                         .arg(offTick));
        scoreInfo.append(QString(", \"x\":%1, \"y\": %2").arg(pos.x()).arg(pos.y()));
        scoreInfo.append(QString(", \"w\":%1, \"h\": %2},").arg(note->width()).arg(note->height()));
        //itufo end
...
static void collectNote(EventMap* events, int channel, const Note* note, int velo, int tickOffset)
{
        ...
        for (int i = 0; i < nels; ++i) {
            ...
            //itufo
            collectTimeline(note, p, velo, on, off);
            if (tieBack && nels == 1)
                break;
            //
...
void Score::renderStaff(EventMap* events, Staff* staff)
    {
        ...
        int mLastEndTick = 0; //itufo 时间线上前一小节的截至时间
        foreach (const RepeatSegment* rs, *repeatList()) {
            int startTick  = rs->tick;
            int endTick    = startTick + rs->len;
            int tickOffset = rs->utick - rs->tick;
            for (Measure* m = tick2measure(startTick); m; m = m->nextMeasure()) {
                Fraction ts = m->timesig(); //itufo
                Fraction len = m->len();//itufo
                
                int mStartTick = mLastEndTick;
                int mEndTick = mStartTick + m->endTick() - m->tick();
                int mStartTime = utick2utime(mStartTick)*1000;
                int mEndTime = utick2utime(mEndTick)*1000;
                
                mLastEndTick = mEndTick;
                scoreInfo.append(QString("\n{\"id\":%1, \"timesig\":[%2,%3], \"len\":[%4,%5] ,\"tick\":[%6,%7], \"time\":[%8,%9] ,\"notes\":[")
                                 .arg(m->getId())      //itufo
                                 .arg(ts.numerator())      //itufo
                                 .arg(ts.denominator())    //itufo
                                 .arg(len.numerator())     //itufo
                                 .arg(len.denominator())   //itufo
                                 .arg(mStartTick)          //itufo
                                 .arg(mEndTick)           //itufo
                                 .arg(mStartTime)         //itufo
                                 .arg(mEndTime));          //itufo
                //if(m->getId() == 837){
                //    m = m;
                //}
